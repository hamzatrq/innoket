...
//int64_t enforceMasternodePaymentsTime = 4085657524; // Innoket
...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleInnoket(name.begin()+1, name.end());
            if (mapArgs.count(singleInnoket) == 0)
                mapArgs[singleInnoket] = entry.second;
            name = singleInnoket;
        }
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Innoket
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Innoket
    // Mac: ~/Library/Application Support/Innoket
    // Unix: ~/.innoket
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Innoket";
#else
    fs::path pathRet;
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty innoket.conf if it does not excist
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Innoket will not work properly.");
