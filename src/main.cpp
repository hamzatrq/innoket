...
#if defined(NDEBUG)
# error "Innoket cannot be compiled without assertions."
#endif
...
static const int64_t nInterval = 1; // Innoket: retarget every block
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    // current difficulty formula, innoket - DarkGravity v3, written by Evan Duffield - evan@innoketpay.io 
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (retarget == DIFF_BTC)
        {
            ...
            // Innoket: This fixes an issue where a 51% attack can change difficulty at will.
