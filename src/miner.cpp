...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("InnoketMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InnoketMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("InnoketMiner started\n");
    ...
    try { while (true) {
        ...
        LogPrintf("Running InnoketMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("InnoketMiner terminated\n");
        throw;
    }
}
